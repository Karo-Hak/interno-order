...
import { DerivedBuyer, FilterMode, SortDir, SortKey, StretchBuyerModel } from './types';
import { compare, derive, normalize, sortIndicator } from './utils';

type Props = {
  rows: StretchBuyerModel[];
  onRowClick?: (buyerId: string) => void;
  onRefresh?: () => void;
};

export const StretchBuyerTable: React.FC<Props> = ({ rows, onRowClick, onRefresh }) => {
  // фильтры и состояние
  ...
  const filtered = React.useMemo(() => {
    ...
    return derived.filter((r) => {
      const nameOk = q ? normalize(r.buyerName).includes(q) : true;
  ...
  return (
    <div className="addStretchBuyer_table">
      ...
      {sorted.length === 0 ? (
        ...
        // ⬇️ прокручиваемый контейнер таблицы
        <div
          style={{
            maxHeight: '70vh',     // нужная высота области прокрутки
            overflow: 'auto',
        ...
        >
          <table className="table" style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0 }}>
            <thead>
              <tr>
                {/* на КАЖДОМ th — position: sticky; top: 0 */}
                <th style={{
                  width: 36,
                ...
                <th onClick={() => onSort('buySum')} style={{
                  ...
                  Գնումներ (քանակ / գումար) {sortIndicator(sortKey === 'buySum', sortDir)}
                </th>
                ...
                <th onClick={() => onSort('creditSum')} style={{
                  ...
                  Վճարումներ (քանակ / գումար) {sortIndicator(sortKey === 'creditSum', sortDir)}
                </th>
                ...
                <th onClick={() => onSort('total')} style={{
                  ...
                  Ընդհանուր մնացորդ {sortIndicator(sortKey === 'total', sortDir)}
                </th>
              </tr>
            </thead>
