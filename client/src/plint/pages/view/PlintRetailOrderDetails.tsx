...
const ORDER_URL = '/plint-retail-order';
...
const money = (v?: number) => n(v).toLocaleString();
...
const PlintRetailOrderDetails: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const [cookies] = useCookies(['access_token']);
  const navigate = useNavigate();

  const [order, setOrder] = React.useState<RetailOrderDto | null>(null);
  ...
  const handleExportPdf = async () => {
    ...
    pdf.save(`plint_retail_order_${id}.pdf`);
...
export default PlintRetailOrderDetails;
